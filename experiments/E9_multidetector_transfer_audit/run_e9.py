#!/usr/bin/env python3
# AUTO-GENERATED from E9_multidetector_transfer_audit.ipynb by pipeline/convert_notebooks.py -- do not edit by hand.
# This is the executable Python conversion of the original Jupyter notebook.
# Standalone:  python experiments/<dir>/run_e9.py
# Via runner:  python main.py --experiment E9
"""E9: converted notebook runner (offline-aware, centralized outputs)."""
...
if __name__ == "__main__":
    ...
    # into the centralized artifacts/E9/ tree.
    os.chdir(exp_output_dir("E9", ""))
    ...
    def make_detector(name: str, seed: int):
    ...
    def build_split_pool(setting: str, seed: int):
    ...
    metric_cols = ['f1', 'precision', 'recall', 'auroc', 'avg_precision',
    ...
    # Wide LaTeX table generation. We use raw strings throughout so backslashes
    ...
    def _fmt_pair(mean, std, kind):
        ...
        if kind == 'f1':
            ...
            return f'{mean:.2f}{{\\scriptsize$\\pm${s:.2f}}}'
        elif kind == 'pct':
            s = 0.0 if pd.isna(std) else float(std)
            return f'{100*mean:.0f}{{\\scriptsize$\\pm${100*s:.0f}}}\\%'
        return f'{mean:.3f}'
