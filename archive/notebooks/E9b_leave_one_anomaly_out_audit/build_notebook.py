"""Builds the E9b leave-one-anomaly-type-out audit notebook.

Run::

    python experiments/E9b_leave_one_anomaly_out_audit/build_notebook.py

This regenerates ``E9b_leave_one_anomaly_out_audit.ipynb`` next to this file.

The notebook addresses reviewer concern M1 by replicating the E9 origin-aware
audit on every TelecomTS anomaly type in turn (each type takes a turn as the
held-out "pseudo-real" origin). It then runs a small calibration mini-sweep on
three representative held-out types to test whether the ~10%-budget repair we
saw on real Jamming generalises beyond Jamming.
"""
...
def build() -> Path:
    nb = NotebookBuilder("E9b: Leave-one-anomaly-type-out paired stress test")

    nb.md(r"""# E9b — Leave-one-anomaly-type-out paired stress test
...
""")
    ...
    nb.md("## 1. Load corpus and engineered features")
...
""")
    ...
    nb.md("""## 2. Per-type LOO split builder
...
""")
    ...
    nb.code("""def build_loo_split(held_out_type: str, seed: int):
...
""")
    ...
    nb.md("## 3. LOO sweep over all 11 anomaly types × 10 seeds")
...
""")
    ...
    nb.md("## 4. Bootstrap 95% CIs on held-out recall and aggregate summary")

    nb.code("""def boot_ci_mean(values, n_boot=2000, seed=0, alpha=0.05):
    rng = np.random.default_rng(seed)
...
""")
    ...
    nb.md("## 5. Headline LOO figure")

    nb.code("""# Build a horizontal bar chart of held-out recall and trained recall side by side.
SHORT = {
    'Jamming'                                  : 'Jam',
...
""")
    ...
    nb.md("""## 6. Calibration mini-sweep on 3 representative held-out types
...
""")
    ...
    nb.code("""CALIB_TYPES   = ['Jamming', 'Co-Channel Interference (Severe)', 'Doppler Shift (Severe)']
...
""")
    ...
    nb.md("## 7. Calibration figure")

    nb.code("""fig, ax = plt.subplots(figsize=(4.8, 2.7))
...
""")
    ...
    out = HERE / "E9b_leave_one_anomaly_out_audit.ipynb"
    return nb.write(out)
