"""Builds the S1 SpotLight cross-corpus distributional check notebook.

Run::

    python experiments/S1_spotlight_origin_distributional_check/build_notebook.py

This regenerates ``S1_spotlight_origin_distributional_check.ipynb`` next to this file.

The notebook addresses reviewer concern M1 (single-benchmark generalizability) by
replicating the TelecomTS operating-regime audit (E2) on the independent SpotLight
Open RAN corpus (Sun et al., MobiCom '24).
"""
...
def build() -> Path:
    nb = NotebookBuilder("S1: SpotLight cross-corpus distributional check")

    nb.md(r"""# S1 — SpotLight cross-corpus distributional check
...
""")
    ...
    nb.md("## 1. Load SpotLight windows and concatenate splits")

    nb.code("""# Concatenate train+val+test so we have the full anomaly population for the audit.
...
""")
    ...
    nb.code("""rf_mask    = (types_all == 'RADIO')
...
""")
    ...
    nb.md("""## 3. Per-window feature representation
...
""")
    ...
    nb.code("""# Per-window mean of each channel: (n_windows, n_channels)
...
""")
    ...
    nb.md("## 4. Per-channel distributional gap (KS / Cohen's d / Wasserstein)")

    nb.code("""rng = np.random.default_rng(SEED)
...
""")
    ...
    nb.code("""# Save per-KPI effect-size table.
...
""")
    ...
    nb.md("## 5. Classifier two-sample test (5-fold HGB on (mean, std, min, max) features)")

    nb.code("""# Build the (n_windows, 4*K) feature matrix on the kept channels only.
...
""")
    ...
    nb.md("## 6. Maximum Mean Discrepancy (RBF kernel, permutation null)")

    nb.code("""# Standardise features for kernel-bandwidth stability.
...
""")
    ...
    nb.md("## 7. Save aggregate two-sample summary")
...
""")
    ...
    nb.md("## 8. Top-effect-size figure (analogue of `figures/E2_top_kpi_effect_sizes.pdf`)")
...
""")
    ...
    nb.md("""## 9. One-sentence interpretation

This cell prints a short paragraph the paper can quote directly.""")

    nb.code("""sinr_max_d  = float(sinr_rows['cohens_d'].abs().max()) if len(sinr_rows) else float('nan')
...
""")
    ...
    out = HERE / "S1_spotlight_origin_distributional_check.ipynb"
    return nb.write(out)
